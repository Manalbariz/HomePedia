...
**Mocks annonces (API)** : `apps/api/data/listings.json` — **22 annonces** (léger, versionné avec le code API). Champs : titre, prix, source (`seloger` / `leboncoin` / `bienici` / `example`), `url`, position carte (`mapX` / `mapY`).
